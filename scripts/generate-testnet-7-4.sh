...
halled add-genesis-account $(hallecli keys show mykey7 -a) 10000000000000000000000000000uhale --home testnet/node0
...
if [[ $os =~ $mac ]];then
    ...
    gsed -i '175,175d' testnet/node9/config/config.toml
    gsed -i "174a persistent_peers = \"$peers\"" testnet/node9/config/config.toml
    gsed -i '175,175d' testnet/node10/config/config.toml
    gsed -i "174a persistent_peers = \"$peers\"" testnet/node10/config/config.toml
else
...
cp scripts/start.sh testnet/start.sh
